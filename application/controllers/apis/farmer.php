...
class farmer extends CI_Controller {
	...
	public function send_forgot_pass_token(){
		...
		if($res['status']){
            ...
            $_mail['message'] = 'Su token generado para la contraseña olvidada es '.$token;
	...
	public function getallfarmers(){
	    exit(json_encode($this->user->getallfarmers()));
	}
    ...
    public function farmer_profile_img_up(){
        // to pass json inputs
        // $inputs = json_decode($this->input->raw_input_stream, true);
    ...
    public function get_uploaded_prod_by_farmer(){
        $res = $this->pro->get_uploaded_prod_by_farmer($this->input->get('uid'));
        exit(json_encode($res));
    }
    ...
    public function updaddress(){
        // to pass json inputs
        $inputs = json_decode($this->input->raw_input_stream, true);
        $res = $this->user->upd_farmer_address($inputs);
        exit(json_encode($res));
    }
    
    public function getfarmerbyid(){
        exit(json_encode($this->user->getfarmerbyid($this->input->get('uid'))));
    }
    
    public function order_got_by_farmer(){
        exit(json_encode($this->ord->order_got_by_farmer(['uid'=>$this->input->get('uid')])));
    }
    
    public function change_ord_status(){
        ...
        $res = $this->ord->change_ord_status($params);
        exit(json_encode($res));
        
    }
    
    public function farmer_order_dets(){
        exit(json_encode($this->ord->farmer_order_dets($this->input->get())));
    }
    
    
    // Forgot password with web.
	public function send_forgot_pass_token_web(){
    	$inputs = json_decode($this->input->raw_input_stream, true);
		...
		if($res['status']){
            ...
            $_mail['to'] = $params['email'];
            $_mail['subject'] = "Se te olvidó tu contraseña";   //"Forgot password";
            $_mail['message'] = 'Su token generado para la contraseña olvidada es <b>'.$token.'</b>.<br/><br/>Por favor, vaya a este sitio para establecer una nueva contraseña. <br/>'.$pass_site_name;
